...
def _request(url, params):
    key = get_key()
    if not key: raise RuntimeError("Missing Spoonacular API key.")
    p = dict(params or {}); p["apiKey"] = key
    r = requests.get(url, params=p, timeout=30)
    if r.status_code == 402:
        raise RuntimeError("Spoonacular quota reached (HTTP 402).")
...
def complex_search(query="", number=10, offset=0, max_ingredients=None, max_ready_time=None, diet=None, intolerances=None, add_nutrition=True):
    params = {"query": query or "dinner", "number": number, "offset": offset, "instructionsRequired": True, "sort": "popularity"}
    if add_nutrition: params["addRecipeNutrition"] = True
